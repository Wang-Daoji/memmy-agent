...
function createSource(db: Database.Database): OpencodeSource | null {
  ...
  return {
    sessions: () => (sessions.all() as Array<Record<string, unknown>>).map(toOpencodeSession),
    messages: (sessionId) => (messages.all(sessionId) as Array<{ id: string; data: string }>)
      .map((row) => ({ id: row.id, data: parseJson(row.data) })),
    parts: (messageId) => (parts.all(messageId) as Array<{ id: string; data: string }>)
      .map((row) => ({ id: row.id, data: parseJson(row.data) }))
  };
}
