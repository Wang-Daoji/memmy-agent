...
def _make_del(text: str, cid: int, when: str) -> etree._Element:
...
def _make_ins(text: str, cid: int, when: str) -> etree._Element:
...
def _replace_in_text_node(
    t_node: etree._Element, old: str, new: str, cid_start: int, when: str
) -> tuple[int, bool]:
    txt = t_node.text or ""
    if old not in txt:
    ...
    d = _make_del(old, cid_start, when)
    cid_start += 1
    ins = _make_ins(new, cid_start, when)
    cid_start += 1
...
def add_tracked_replacements(
    in_docx: str, out_docx: str, replaces: list[tuple[str, str]]
) -> None:
    ...
    with zipfile.ZipFile(in_docx, "r") as zin:
        ...
        for old, new in replaces:
            for t in doc_root.xpath(".//w:t", namespaces=NS):
                cid, changed = _replace_in_text_node(t, old, new, cid, when=when)
                if changed:
                    total += 1
                    break
...
def main() -> None:
    ...
    add_tracked_replacements(args.in_docx, args.out, replaces)
