...
def _default_macos_tmpdir_for_soffice() -> None:
    ...
    Some desktop launchers provide a per-process TMPDIR that causes headless
...
def _prepend_bundled_runtime_bin() -> None:
    ...
    configured_runtime_bin = os.environ.get("MEMMY_DOCX_RUNTIME_BIN", "")
    if configured_runtime_bin:
        runtime_bins.insert(0, configured_runtime_bin)
...
def _build_lo_env(user_profile: str) -> dict:
    """Build an isolated environment for LibreOffice.
