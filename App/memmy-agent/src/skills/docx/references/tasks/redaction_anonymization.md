...
This bundle provides `scripts/redact_docx.mjs`, which redacts by editing OOXML text nodes while attempting to preserve layout:
...
2. Run `redact_docx.mjs` with carefully scoped patterns.
...
```bash
node scripts/redact_docx.mjs /mnt/data/input.docx \
