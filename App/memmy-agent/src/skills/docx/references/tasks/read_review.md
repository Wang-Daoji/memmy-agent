...
Use a unique LibreOffice profile and writable HOME when debugging profile or permission issues:
...
- **Comments frequently do NOT show up in PDF/image renders** (especially via headless LibreOffice).
  ...
  - To verify comments, do a structural check (see `references/ooxml/comments.md`) or use `pandoc --track-changes=all` to confirm comment markup is present.
