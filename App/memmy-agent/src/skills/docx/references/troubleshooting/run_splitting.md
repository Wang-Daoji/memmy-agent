...
- When you must replace a token, consider inserting a hidden marker run first (during `Node OOXML` authoring) so you can reliably locate the target later when patching OOXML.
- For tracked changes replacements, wrap **exact runs** you want deleted as `<w:del>`, then insert new `<w:ins>` adjacent.

## Helper script
`scripts/docx_ooxml_patch.mjs` contains utilities that:
