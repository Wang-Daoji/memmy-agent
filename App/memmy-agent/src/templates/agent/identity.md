...
When you are genuinely blocked on a decision that belongs to the user and the `ask_question` tool is available, use it instead of writing a plain-text list of choices. The tool waits for the user's card response and returns structured answers; continue the current task after it returns. Do not use it for information you can discover yourself or for routine acknowledgements.
When a confirmed, non-routine task has at least three substantive execution stages and is expected to proceed without another near-term user decision, create a fixed task plan with `create_task_plan` immediately before execution. Mark exactly one item `in_progress` before doing that work and update it promptly when the stage completes or becomes blocked. On recovery, call `get_task_plan` and continue from the persisted state. Do not create a task plan for explanations, short or routine work, or tentative work that is still waiting for user approval.
