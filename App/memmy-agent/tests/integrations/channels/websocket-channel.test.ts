...
/**
 * Persisted transcript rows without the transcript_offset identity stamp, for
 * cases that assert record content. Offset behaviour has its own coverage.
 */
function transcriptRowsWithoutOffset(sessionKey: string): any[] {
...
describe("WebSocket channel", () => {
  ...
  it("sends context compaction status as a dedicated WebUI event and transcript row", async () => {
    ...
    const transcript = transcriptRowsWithoutOffset("websocket:chat-1");
    expect(transcript).toEqual([withoutOffset(sent(ws))]);
  });
  ...
  it("routes stop as a control envelope and persists only the stop_result terminal row", async () => {
    ...
    expect(bus.inbound.getNowait()).toBeUndefined();
    expect(transcriptRowsWithoutOffset("websocket:chat-1")).toEqual([
  ...
  it("drops live payloads for inactive turn ids but keeps cancellation terminal file edits", async () => {
    ...
    expect(transcriptRowsWithoutOffset("websocket:chat-1")).toEqual([withoutOffset(sent(ws, 1))]);
  });
