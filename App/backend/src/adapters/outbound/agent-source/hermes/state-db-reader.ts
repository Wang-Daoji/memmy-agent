...
function createSource(db: DatabaseSync): HermesSource | null {
  ...
  return {
    sessions: () => sessions.all() as unknown as Array<{ id: string; cwd: string | null }>,
    messages: (sessionId) => messages.all(sessionId) as unknown as HermesRow[]
  };
}
...
function tableColumns(db: DatabaseSync, tableName: string): ReadonlySet<string> {
  return new Set((db.prepare(`PRAGMA table_info("${tableName}")`).all() as unknown as Array<{ name: string }>).map((row) => row.name));
}
