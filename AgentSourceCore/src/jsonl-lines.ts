 ...
 */
export async function* readJsonlObjects(filePath: string, signal?: AbortSignal, onInvalidRecord?: (reason: string) => void): AsyncIterable<JsonObject> {
  ...
  const parseSegments = (): JsonObject | null => {
    if (overLimit) {
      onInvalidRecord?.("source_record_over_limit");
      return null;
    }
    ...
    try {
      ...
      onInvalidRecord?.("source_record_invalid");
      return null;
    } catch {
      onInvalidRecord?.("source_record_invalid");
      return null;
