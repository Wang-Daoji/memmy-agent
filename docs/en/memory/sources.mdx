...
- **Task resumption:** `/memmy-resume <query>` returns up to five L1 episode candidates. Enter `1`–`5` to load the complete episode and inject continuation context. Claude Code, OpenCode, OpenClaw, and Hermes use native command entry points; Cursor, Codex, and Skill-only integrations install an explicit `memmy-resume` Skill.
...
6. Enter `/memmy-resume <keywords>`, confirm that candidates appear, and select an episode with `1`–`5`.
