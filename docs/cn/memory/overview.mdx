...
默认 `algorithm.retrieval.filterBackend` 是 `llm`。机械排序后先用 summary 模型做相关性过滤；这次调用失败才改用 evolution。设为 `jev` 时，这一步只调用 Jev `jev-1.13.0`，不调用 summary 或 evolution。Jev 的 key 来自 `MEMMY_JEV_API_KEY` 或 `memmyMemory.jev.apiKey`。

`llm` 且 summary、evolution 都未配置时，状态是 `llm_filter:no_llm`，保留机械排序的前 `llmFilterFallbackMaxKeep` 条。`jev` 且没有 key 时也是 `llm_filter:no_llm`，不改打 summary。`jev` 调用失败或响应无效时状态是 `llm_filter:llm_failed_fallback_cap`，同样按保底上限截断，不改打原来的 LLM。

| 配置 | 默认值 | 作用 |
| --- | ---: | --- |
| `llmFilterEnabled` | `true` | 是否启用最终语义过滤 |
| `llmFilterMinCandidates` | `1` | 至少有多少条机械候选才调用 LLM |
